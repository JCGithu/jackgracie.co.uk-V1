{
    class Details {
        constructor() {
            ...
            <video class="details__img" src="" loop autoplay alt="img 01"></video>
        ...
        fill(info) {
            ...
            this.DOM.img.dataset.id = info.moCode;
        }
        getmoDetailsRect() {
            return {
                moBgRect: this.DOM.moBg.getBoundingClientRect(),
                detailsBgRect: this.DOM.bgDown.getBoundingClientRect(),
                moImgRect: this.DOM.moImg.getBoundingClientRect(),
        ...
        open(data) {
            ...
            this.DOM.moBg = data.moBg;
            this.DOM.moImg = data.moImg;
            this.DOM.youtube = data.youtube;

            this.DOM.moBg.style.opacity = 0;
            this.DOM.moImg.style.opacity = 0;

            const rect = this.getmoDetailsRect();

            this.DOM.bgDown.style.transform = `translateX(${rect.moBgRect.left-rect.detailsBgRect.left}px) translateY(${rect.moBgRect.top-rect.detailsBgRect.top}px) scaleX(${rect.moBgRect.width/rect.detailsBgRect.width}) scaleY(${rect.moBgRect.height/rect.detailsBgRect.height})`;
            this.DOM.bgDown.style.opacity = 1;
            
            this.DOM.img.style.transform = `translateX(${rect.moImgRect.left-rect.detailsImgRect.left}px) translateY(${rect.moImgRect.top-rect.detailsImgRect.top}px) scaleX(${rect.moImgRect.width/rect.detailsImgRect.width}) scaleY(${rect.moImgRect.height/rect.detailsImgRect.height})`;
        ...
        close() {
            ...
            anime({
                ...
                translateX: (target, index) => {
                    return index ? rect.moImgRect.left-rect.detailsImgRect.left : rect.moBgRect.left-rect.detailsBgRect.left;
                },
                translateY: (target, index) => {
                    return index ? rect.moImgRect.top-rect.detailsImgRect.top : rect.moBgRect.top-rect.detailsBgRect.top;
                },
                scaleX: (target, index) => {
                    return index ? rect.moImgRect.width/rect.detailsImgRect.width : rect.moBgRect.width/rect.detailsBgRect.width;
                },
                scaleY: (target, index) => {
                    return index ? rect.moImgRect.height/rect.detailsImgRect.height : rect.moBgRect.height/rect.detailsBgRect.height;
                },
                complete: () => {
                    ...
                    this.DOM.moBg.style.opacity = 1;
                    this.DOM.moImg.style.opacity = 1;
        ...
        zoomIn() {
            ...
            if ( win.w > 0.8*win.h ) {
                ...
                Object.assign(imgAnimeOpts, {
                    ...
                    scaleY: 0.5*win.w/parseInt(0.8*win.h),
                });
            } */
	    ...
	    if ( this.isAnimating ) return false;
            ...
            if (screen.width < screen.height ) {
                ...
                Object.assign(imgZoomOutOpts, {
                    right: '-305vw',
                });
            }
            
            anime(imgZoomOutOpts);
    ...
    class Item {
        constructor(el) {
            this.DOM = {};
            this.DOM.el = el;
            this.DOM.mo = this.DOM.el.querySelector('.mo');
            this.DOM.moBg = this.DOM.mo.querySelector('.mo__bg');
            this.DOM.moImg = this.DOM.mo.querySelector('.mo__img');
            this.DOM.moDeco = this.DOM.mo.querySelector('.mo__add');

            this.info = {
                img: this.DOM.moImg.src,
                title: this.DOM.mo.querySelector('.mo__title').innerHTML,
                subtitle: this.DOM.mo.querySelector('.mo__subtitle').innerHTML,
                description: this.DOM.mo.querySelector('.mo__description').innerHTML,
                price: this.DOM.mo.querySelector('.mo__price').innerHTML,
                decoCol: this.DOM.moDeco.style.backgroundColor,
                fonts: this.DOM.moDeco.style.fontSize,
                moCode: this.DOM.moDeco.dataset.id,
            };

            this.initEvents();
        }
        initEvents() {
            this.DOM.mo.addEventListener('click', () => this.open());
        }
        open() {
            DOM.details.fill(this.info);
            DOM.details.open({
                moBg: this.DOM.moBg,
                moImg: this.DOM.moImg,
            });
        }
    };
